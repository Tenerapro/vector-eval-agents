...
2. grounding_to_citations
   - Value must be 1 only if the candidate answer is supported by the citations it provides and those citations are relevant to the question.
   - Otherwise value must be 0.
3. completeness
   - Value must be 1 only if the candidate answer includes the important information needed to answer the input.
   - Otherwise value must be 0.
4. citation_usefulness
   - Value must be 1 if the citations are specific enough to help a reviewer verify the answer.
   - Otherwise value must be 0.
5. caveat_quality
